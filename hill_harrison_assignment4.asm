...
findword:       #findword increments our address until it finds the start of the next word
...
newlargest:
    or $t6, $0, $v0       #$t6 now holds the current largest size
    or $t7, $0, $a0       #$t7 now holds the address of the current largest word

eatinput:       #here we increment our address until we find the next non-word, then pass back to findword
...
endfindmax:
    or $a0, $0, $t5     #set $a0 to address of allocated space
    or $a1, $0, $t7     #set $a1 to address of largest word

    addi $sp, $sp, -4
    sw $ra, 0($sp)
    jal strcpy                      #call strcpy to copy largest word into allocated space
    lw $ra, 0($sp)
    addi $sp, $sp, 4
...
# $t3 - size counter
# $t2 - holds initial value of $a0 (the starting address of string)
# $a0 - used to store characters so they can be passed to isletter
